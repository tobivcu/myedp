...
//Test NewQueue and Dequeue
/*	char * test ="hello";
	Queue* test1 = NewQueue();
	if(test1 == NULL)
		printf("test1 is NULL\n");
	else
		printf("test1 is not NULL and the length of test1 is %d\n", test1->cur_len);
	Enqueue(test1, test);
	if(test1 == NULL)
                printf("test1 is NULL\n");
        else
	...
	for(i=0;i<map_cnt;i+=WINDOW_SIZE){
		...
		//	printf("after enqueue\n");
			if(cargs[j].q == NULL)
				...
				{printf("cargs[j].q is not NULL and the current length of the queue is %d\n", cargs[j].q->cur_len);}
